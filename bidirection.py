...
def bidirectional_bfs(graph, start, goal):
    ...
    while queue_start and queue_goal:
        ...
        # Backward BFS step (traverse incoming edges)
        curr_goal = queue_goal.popleft()
        for neighbor in (node for node, nbrs in graph.items() if curr_goal in nbrs):
...
for p in s.split():
    ...
    if neighbors:
        graph[node] = [n for n in neighbors.split(",") if n]
    else:
        graph[node] = []
