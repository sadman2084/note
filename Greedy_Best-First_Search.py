...
def greedy_bfs(start, goal):
    pq = [(heuristic[start], 0, start)]   # (heuristic, cost_so_far, node)
    visited = set()

    while pq:
        h, cost, node = heapq.heappop(pq)
        if node in visited:
            continue
        ...
        if node == goal:
            print("Goal reached!")
            print("Total cost from", start, "to", goal, "=", cost)
            return

        for neighbor, edge_cost in graph[node]:
            if neighbor not in visited:
                heapq.heappush(pq, (heuristic[neighbor], cost + edge_cost, neighbor))
